...
class StandardCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Block 4
        x = F.relu(self.bn7(self.conv4(x)))
        x = F.relu(self.bn8(self.conv4b(x)))
        
        # Block 5
        x = F.relu(self.bn9(self.conv5(x)))
        x = F.relu(self.bn10(self.conv5b(x)))
        
        # Block 6
        x = F.relu(self.bn11(self.conv6(x)))
        x = F.relu(self.bn12(self.conv6b(x)))
...
def train_model_single_run(model, train_loader, val_loader, num_epochs=25, device='cuda', verbose=False):
...
def analyze_kernel_selection_single_run(model, test_loader, device='cuda'):
    model.eval()
    kernel_selections = {0: [], 1: [], 2: [], 3: [], 4: [], 5: []}  # 6 adaptive layers
    ...
    with torch.no_grad():
        for data, _ in test_loader:
            ...
            for layer_idx, weights in enumerate(model.attention_weights):
                if layer_idx < 6:  # 6 adaptive layers
...
def run_experiment(seeds=[42, 123, 456, 789, 999], num_epochs=25):
    ...
    for i, seed in enumerate(seeds):
        ...
        adaptive_model = AdaptiveReceptiveFieldCNN(num_classes=10)
        ...
        standard_model = StandardCNN(num_classes=10)
