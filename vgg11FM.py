...
Modified for Fashion-MNIST dataset with 25 epochs
"""
...
class AdaptiveVGG11(nn.Module):
    def __init__(self, num_classes=10):  # Changed to 10 for Fashion-MNIST
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(4096, 4096),
            nn.ReLU(True),
            nn.Linear(4096, num_classes),  # Changed to num_classes for Fashion-MNIST
        )
...
def train_model_single_run(model, train_loader, val_loader, num_epochs=25, device='cuda', verbose=False):  # Changed default to 65
...
def plot_results(results):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss (Fashion-MNIST)')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Validation Accuracy (Fashion-MNIST)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Training Accuracy (Fashion-MNIST)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Validation Loss (Fashion-MNIST)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.title('VGG11 Kernel Selection Patterns (Fashion-MNIST)')
    ...
    plt.title('VGG11 Kernel Selection Patterns (First 5 Layers, Fashion-MNIST)')
    ...
    print(f"\nSUMMARY STATISTICS (Fashion-MNIST):")
...
def run_experiment(seeds=[42, 123, 456, 789, 999], num_epochs=25):  # Changed default to 65
    ...
    try:
        os.makedirs('results', exist_ok=True)
        with open('results/vgg11_fashion_mnist_results.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    results = run_experiment(seeds=[42, 123, 456, 789, 999], num_epochs=25)